...
handlebars.registerHelper('smw_templ_attr', function(attr) {
    ...
    if (format && (format.charAt(0) === '/' || format.toLowerCase() === 'page')) {
        isPage = true;
    }
    ...
    if (attr.smw_arraymap) {
        ...
        //result = '{{#arraymap:{{{' + attributeName + '|}}}|;|@@@@|[[' + attributeName + '::' + prefix + '@@@@' + suffix + ']]|,&nbsp;}}';
        ...
        //result = '[[' + attributeName + '::' + prefix + '{{{' + attributeName + '|}}}' + suffix + ']]';
...
handlebars.registerHelper('smw_subobject', function(attr, name) {
    ...
    // Automatically inserted dummy attribute, for easier querying
    // @see https://semantic-mediawiki.org/wiki/Help:Adding_subobjects#Querying_for_subobjects
    // Example: [[subobject=HauptStandortAdresse]][[Superobject::Heuchler, Erich]]
