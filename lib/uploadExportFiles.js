    ...
    if (uploadCounter > 0) {
        ...
        var handleUploadResponse = function(response) {
            ...
            if (response.result === 'Success') {
                log(' U (' + exports.pad(uploadStatus, 3) + '/' + exports.pad(uploadCounter, 3) + ') | ' + response.title);
                uploadStatus++;
            } else {
        ...
        formbot.logIn(botConfig.username, botConfig.password, function(response) {
            ...
            if (response.result === 'Success') {
                ...
                for (siteName in uploadQueue) {
                    ...
                    formbot.edit(siteName, siteContent, 'FormBot Edit', handleUploadResponse(response));
                }
