    ...
    if (Object.keys(uploadMap).length === 0) {
        ...
        exports.bot.logIn(botConfig.username, botConfig.password, function() {
            ...
            for (var pageName in uploadMap) {
                ...
                exports.bot.api.call({action: 'query', titles: pageName}, function(err, info) {

                    if (!settings.overwriteImportedPages && info && info.pages && !info.pages['-1']) {
                        exports.counter += 1;
                        log('[D] (' + semlog.pad(exports.counter, 5) +
                            ' / ' + semlog.pad(exports.total, 5) + ') Skipping page because of "overwriteImportedPages"');
                    } else {
