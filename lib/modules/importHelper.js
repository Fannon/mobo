...
exports.enhanceWithForm = function(objCollection, formSchema) {
    ...
    if (formSchema && formSchema.$path) {
        ...
        log(' ');
        log(' [i] Enhancing import collection with ' + formSchema.$path);
        ...
        for (var templateName in formSchema.properties) {
            ...
            if (template.$reference && template.$reference.type === 'smw_template') {
                ...
                if (item.length === 1) {
                    ...
                    log(' [W] Inserting add a singule instance template more than once: ' + template.id);
                }
                ...
                var items = exports.findInCollection(objCollection, template.id);

                for (var i = 0; i < items.length; i++) {
                    var multipleInstanceTemplate = items[i];
                    log(' [D] Inserting multiple instance template: ' + template.id);
                    newCollection.push({
                        name: template.id,
                        template: multipleInstanceTemplate
                    });
                    inserted.push(template.id);
                }
        ...
        for (var j = 0; j < objCollection.length; j++) {
            ...
            if (obj.name && inserted.indexOf(obj.name) === -1) {
                ...
                log(' [D] Appending additional item: ' + name);
