...
exports.mergeObjects = function(obj1, obj2) {
    return _.merge(obj1, obj2, function(objectValue, sourceValue, key) {
        ...
        if (_.isArray(objectValue)) {
            ...
            var prepend = exports.settings.mergeArrays;

            // Check for annotations that define how arrays should be handled
            if (objectValue.indexOf('@prepend') > -1) {
                prepend = true;
            }
            ...
            _.pull(objectValue, '@overwrite', '@prepend', '@append', '@unique', '@sorted');
            ...
            // @unique - removes all duplicate values
            if (unique) {
                merged = _.uniq(merged);
            }

            // @sorted - sorts the resulting array alphabetically
            if (sorted) {
                merged = merged.sort();
            }
