...
exports.generateForm = function(json, name, settings) {
    ...
    for (var modelName in json.properties) {
        ...
        if (model) {
            ...
            if (model.smw_prefix && model.smw_prefix.showForm) {
                if (model.smw_prefix.wikitext ) {
                    templatingModel.prefix = model.smw_prefix.wikitext;
                } else if (model.smw_prefix.header) {
                    // TODO: Support h1 - h4
                    var headerLevel = '=';
                ...
                log(templatingModel.prefix);
            }

            if (model.smw_postfix && model.smw_postfix.wikitext && model.smw_postfix.showForm) {
