...
exports.exec = function(settings, registry) {
    ...
    return new Promise(function (resolve) {
        ...
        exports.validate(exports.nodes, exports.edges);
...
exports.validate = function(nodes, edges) {
    ...
    // Check that every node is connected somehow (not an orphan)
    for (var nodeName in nodes) {
        ...
        if (!hasConnections) {
            ...
            if (nodeName.indexOf("template:") > -1) {
                ...
                if (exports.settings.verbose) {log(node);}
            }
    ...
    // TODO: This is sometimes broken or unnecessary through the $extend validation
    //for (var edgeName in edges) {
    //    var edge = edges[edgeName];
    //
    //    if (!nodes[edge.source]) {
    //        log(' [W] [Graph] Edge from "' + edge.target + '" is missing its source node "' + edge.source + '"');
    //        if (exports.settings.verbose) {log(edge);}
    //    }
    //
    //    if (!nodes[edge.target]) {
    //        log(' [W] [Graph] Edge from "' + edge.source + '" is missing its target node "' + edge.target + '"');
    //        if (exports.settings.verbose) {log(edge);}
    //    }
    //}
};
