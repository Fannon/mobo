...
exports.generateCategory = function(settings, obj, name, registry) {
    ...
    if (obj.smw_categoryPrefix) {
        data.prefix = obj.smw_categoryPrefix;
    }

    if (settings.autoGenerateSemanticDrilldown) {
                    ...
                    if (model.properties) {
                        for (var fieldName in model.properties) {
                            ...
                            if (field.smw_drilldown) {
                                ...
                                //if (field.smw_form && field.smw_form['values from category']) {
                                //    fieldInfo.category = field.smw_form['values from category'];
                                //}

                                drilldowninfo.push(fieldInfo);
                            }
                        }
                    }
                }
        ...
        for (var i = 0; i < drilldowninfo.length; i++) {
            ...
            drilldowninfoFunction += item.name + ' (property=' + item.id;
            if (item.category) {
        ...
        if (drilldowninfo.length > 0) {
            data.drilldowninfo = drilldowninfoFunction;

            console.log();
            console.log();
            log(drilldowninfoFunction);
            console.log();
        }
