...
exports.generateTemplate = function(settings, obj, name, registry) {
    ...
    for (var propertyName in data.attr) {
        ...
        // Hack because handlebars.js is stupid
        if (data.attr[propertyName].smw_hideInPage === true) {
            data.attr[propertyName].display = false;
        } else {
            data.attr[propertyName].display = true;
        }
    }
    ...
    data.prefix = moboUtil.prePostFix('smw_prefix', 'showPage', obj, registry);
    data.postfix = moboUtil.prePostFix('smw_postfix', 'showPage', obj, registry);
