 ...
 * @param {string}  templateName    name of the template, excluding .wikitext extension
 * @param {{}}      settings
 *
 * @returns {*}
 */
exports.loadTemplate = function(scope, templateName, settings) {
    ...
    if (!template) {
        var filePath = settings.templateDir + templateName + '.wikitext';
        try {
            var file = fs.readFileSync(filePath).toString();
            template = handlebars.compile(file);
        } catch (e) {
            exports.log(' [E] Could not load template: ' + filePath);
        }
    }
