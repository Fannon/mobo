        ...
        for (var i = 0; i < registryTypes.length; i++) {
            ...
            for (var elName in elements) {
                ...
                if (obj.type === 'array' && !obj.items) {
                    log('[E] If the type is "array", "items" must be set. > ' + obj.$path);
                }

                if (obj.smw_form && obj.smw_form['input type'] &&  obj.smw_form['input type'] === 'tokens' && !obj.items) {
                    log('[E] The tokens widget only makes sense with "type": "array". > ' + obj.$path);
                }

                if (obj.default && obj.enum && obj.enum.indexOf(obj.default) < 0) {
                    log('[E] The default value "' + obj.default + '" is not part of the enum. > ' + obj.$path);
                }
