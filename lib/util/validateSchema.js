    ...
    return new Promise(function (resolve) {
        ...
        for (var i = 0; i < registryTypes.length; i++) {
            ...
            for (var elName in elements) {
                exports.validate(elements[elName], moboSchema[type + 'Schema']);
            }
        }
    ...
    return new Promise(function (resolve) {
        ...
        for (var i = 0; i < registryTypes.length; i++) {
            ...
            for (var elName in elements) {
                ...
                var id = obj.$filepath || obj.id || '(unknown)';
                ...
                if (obj.required) {
                    for (var j = 0; j < obj.required.length; j++) {
                        if (!obj.properties[obj.required[j]]) {
                            log(' [W] ' + obj.id + ' requires non existent property "' + obj.required[j] + '"!');
                        }
                    }
                }
            }
