...
* * Plain text
* * Link to WikiPage
* * FormRedLink, using (multiple if oneOf is given) Semantic Forms
*
* @param attr
*/
exports.printProperty = function(attr) {
    ...
    // Add Pre- and Suffix for the Field text / Link URL
    ...
    if (attr.smw_arraymap) {
        ...
        if (attr.smw_arraymaptemplate) {
            result = '{{#arraymaptemplate:{{{' + attributeName + '|}}}|' + attr.smw_arraymaptemplate + '|' + sep + '|}}';
        }
    ...
    // Completely overwrite the attribute display
