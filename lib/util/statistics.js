...
exports.registryStatistics = function(settings, registry) {
    ...
    complexity.processedModelPercentage = Math.round((complexity.processedModelSize / complexity.devModelSize) * 100);
    complexity.generatedPercentage = Math.round((complexity.generatedSize / complexity.devModelSize) * 100);
...
exports.writeStatistics = function(statistics, directory) {
    ...
    // UNIX Timestamp (Microtimestamp / 1000) + Human readable date
    var now = (new Date());
    var header = ['timestamp', 'date'];
    var content = [Math.round(now.getTime() / 1000), moboUtil.humanDate(now)];
