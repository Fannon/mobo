...
exports.exec = function(settings, onGenerated, callback) {
    ...
    if (settings.debug) {log(' [D] [1] READ PROJECT FILES, VALIDATE SYNTAX');}
    ...
    moboTaskQueue.then(function(registry) {
        ...
        if (settings.debug) {log(' [D] [2] VALIDATE PROJECT SCHEMA / STRUCTURE');}
        return validateSchema.validateRegistry(exports.registry);
    });

    // 3) EXTEND PROJECT REGISTRY (applies inheritance)
    moboTaskQueue.then(function(registry){
        if (settings.debug) {log(' [D] [3] EXTEND PROJECT REGISTRY (INHERITANCE)');}
        return extendProject.exec(settings, registry);
    }).catch(function(e) {
        log(' [E] Error while appling inheritance.');
        log(e);
    });

    // 3.5) VALIDATE EXTENDED REGISTRY
    moboTaskQueue.then(function(registry) {
        exports.registry = registry;
        if (settings.debug) {log(' [D] [3.5] VALIDATE PROJECT SCHEMA / STRUCTURE');}
        return validateSchema.validateExpandedRegistry(exports.registry);
    });

    // 2) VALIDATE PROJECT SCHEMA
    if (settings.buildGraph) {
        moboTaskQueue.then(function(registry){
            if (settings.debug) {log(' [D] [4] BUILD GRAPH / VALIDATE SEMANTICS');}
            return generateGraph.exec(settings, registry);
        }).catch(function(e) {
            log(' [E] Error while building the graph structure.');
            log(e);
        });
    }
    ...
    moboTaskQueue.then(function(registry) {
        ...
        // TODO: Make callbacks unecessary
        if (callback) {
            callback(false, registry.generated);
        }

        if (settings.debug) {log(' [D] [6] WRITE EXPORT FILES');}
        return writeExportFiles.exec(settings, registry);

    }).catch(function(e) {
        log(' [E] Error while writing the export files');
        log(e);
    });

    moboTaskQueue.then(function(registry) {
        if (settings.debug) {log(' [D] [7] UPLOAD TO WIKI');}

        // TODO: One last callback
        return uploadExportFiles.exec(settings, registry, function(err) {
            ...
            if (settings.debug) {log(' [D] [8] COMPLETED. WRITING LOG FILES');}
    ...
    // X) HANDLE ERRORS
    // TODO: Handle errors between steps
    ...
    moboTaskQueue.lastly(function() {
        // TODO: Not working
        if (settings.debug) {log(' [D] [X] PROMISE LASTLY');}
    });
