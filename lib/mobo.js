...
exports.exec = function(settings, onGenerated, callback) {
    ...
    moboTaskQueue.then(function(registry) {
        ...
        if (settings.complexityStatistics) {
            exports.complexityStatistics(registry);
        }
...
exports.complexityStatistics = function(registry) {
    ...
    stats.devModelSize = JSON.stringify(devModel).length;
    stats.processedModelSize = JSON.stringify(processedModel).length;
    stats.processedModelPercentage =  Math.round((stats.processedModelSize / stats.devModelSize) * 100);
    stats.generatedSize = JSON.stringify(registry.generated).length;
    stats.generatedPercentage =  Math.round((stats.generatedSize / stats.devModelSize) * 100);

    log(stats, true);

    log(' ' +
        moboUtil.thousandSeparator(stats.devModelSize) + ' DevModel | ' +
        moboUtil.thousandSeparator(stats.processedModelSize) + ' (' +
        stats.processedModelPercentage + '%) ProcModel  | ' +
        moboUtil.thousandSeparator(stats.generatedSize) + ' (' +
        stats.generatedPercentage + '%) Generated');
    log('------------------------------------------------------------------------------');
}