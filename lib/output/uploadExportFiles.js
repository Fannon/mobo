...
exports.exec = function(settings, registry, callback) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        //if (settings.uploadWikiPages === false && settings.forceUpload === false) {
        //    log('NO UPLOAD');
        //    resolve(); // Skip upload
        //}
        ...
        if (!settings.mw_server_url || !settings.mw_username || !settings.mw_password) {
            ...
            return callback(new Error('incomplete login data'), false);
        ...
        if (registry && registry.generated) {
            ...
            try {
                // TODO: Generated Pages could be given as a parameter
                generatedPages = JSON.parse(fs.readFileSync(settings.processedModelDir + '_generated.json').toString());
            } catch (e) {
                log(' [E] Could not read /_processed/_generated.json');
                return callback(e, false);
                //reject(e);
            }
        }

        if (registry && registry.lastUploadState) {
            lastUploadState = registry.lastUploadState;
        } else {
            try {
        ...
        if (uploadCounter > 0 || (settings.deleteWikiPages && removedCounter > 0)) {
                ...
                if (err || !response) { // Handle error object from nodemw
                    ...
                    if (settings.deleteWikiPages) {
                        ...
                        for (pageName in removed) {
                            exports.bot.delete(pageName, 'mobo delete', deletePage);
                        }
                    }
                    ...
                    for (pageName in uploadQueue) {
                        ...
                        exports.bot.edit(pageName, pageContent, 'mobo edit', editPage);
                    }
                }
            };

            var editPage = function(err, response) {
            ...
            var deletePage = function(err, response) {
...
exports.complete = function(currentUploadState, start, callback) {
    ...
    if (currentUploadState) {
        ...
        exports.registry.lastUploadState = currentUploadState;
    }
