...
exports.exec = function(settings, registry) {
    ...
    for (var fieldName in registry.field) {
        ...
        // Validation
        if (!type) {
            log('> [WARNING] [Graph Structure] ' + fieldPageName + ':: No Field Type!');
            if (settings.verbose) {
                log(field);
            }
        }
    ...
    for (modelName in registry.expandedModel) {
        ...
        for (var propertyName in model.properties) {
            ...
            if (!field) {
                ...
                log('> [WARNING] [Graph Structure] Model "' + modelPageName + '" is missing its field "' + fieldPageName + '"!');
                ...
                if (anyArray) {
                    ...
                    log('> [WARNING] [Graph Structure] Field "' + fieldPageName + '" has missing node "' + modelPageName + '"!');
            ...
            for (modelName in models) {
                ...
                if (model) {
                    ...
                    if (modelId) {
                        ...
                        log('> [WARNING] [Graph Structure] Form "' + formPageName + '" has missing template "' + modelPageName + '"!');
                        if (settings.verbose) {log(form);}
                    }
                } else {
                    log('> [WARNING] [Graph Structure] Form "' + formPageName + '" has missing model "' + modelName + '"!');
                    if (settings.verbose) {log(form);}
                }
            }

        } else {
            log('> [WARNING] [Graph Structure] Form "' + formPageName + '" has no properties!');
            if (settings.verbose) {log(form);}
        }
