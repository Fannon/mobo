...
exports.writeSchemas = function() {
    ...
    for (var i = 0; i < schemaExports.length; i++) {
        ...
        schema = exports[schemaName + 'Schema'];

        fileName = dir + '/' + schemaName + '/SCHEMA.md';
        fileContentHeader  = '# ' + schemaName.toUpperCase() + ' JSON SCHEMA\n\n';
        fileContentHeader += 'This is the complete JSON Schema file that will validate all your ' + schemaName + 's.\n';
        fileContentHeader += 'Please look especially for the "description" properties. \n';

        // use ```json for a code listing
        fileContent = fileContentHeader + '```json\n' + JSON.stringify(schema, false, 4) + '\n```';

        fs.outputFile(fileName, fileContent);
    }
};
