...
exports.exec = function(settings) {
    ...
    registry.smw_page = getDirectoryContent(settings.importModelDir + '/smw_page/');

    /** Fallback for @deprecated project structure! */
    if (!registry.smw_page) {
        registry.smw_page = getDirectoryContent(settings.importModelDir + '/smw_site/');
    ...
    if (model.properties && model.propertyOrder) {
        ...
        for (var i = 0; i < model.propertyOrder.length; i++) {
            ...
            if (model.properties[propertyName]) {
                ...
                log('> [WARNING] Model ' + model.title + ' has non existent property "' + propertyName + '" in the propertyOrder array!');
            }
