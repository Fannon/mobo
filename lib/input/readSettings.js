...
exports.exec = function(customSettings) {
    ...
    if (customSettings) {
        ...
        if (fs.existsSync(path.join(process.cwd(), '/settings.yaml'))) {
            ...
            try {
                ...
                log('[E] [SYNTAX] Could not parse project settings: ' + e.message);
                log('[i] Check the settings.yaml/json for valid syntax.');
                log(e, true);
                return false;
            }
    ...
    var valid = validate.validateSettings(finalSettings);

    if (!valid) {
        log('[E] [STRUCTURE] Invalid settings!');
